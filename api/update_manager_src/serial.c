...
int serial_open(int arg)
{
	...
	if (arg==1)
	{
		HMI_DEBUG("open /dev/ttyUSB0\n");
		serial_fd = open("/dev/ttyUSB0", O_RDWR);
	}
	else if (arg == 2)
	{
		HMI_DEBUG("open /dev/ttyS!");
		serial_fd = open("/dev/ttyS1", O_RDWR);
	}
	else
	{
		HMI_ERROR("the arg :[%d] is error!\n", arg);
		return -1;
	}
	...
	{
		return -1;
		HMI_ERROR("serial_open the serial%d error!\n", arg);
	}

		fset = fcntl(serial_fd, F_SETFL, 0);	
		if (fset < 0)		
			HMI_ERROR("fcntl failed!\n");	
		else		
			HMI_DEBUG("fcntl = %d\n", fset);	
#if 0		
...
int serial_setopt(int fd, int nSpeed, int nBits, char nEvent, int nStop)
{
	...
	{
		HMI_ERROR("the nSpeed :[%d] is error!\n", nSpeed);
		return -1;
	}
	if (!(nBits==7 || nBits==8))
	{
		HMI_ERROR("the nBits :[%d] is error!\n", nBits);
		return -1;
	}
	if (!(nEvent=='O' || nEvent=='E' || nEvent=='N'))
	{
		HMI_ERROR("the nEvent :[%c] is error!\n", nEvent);
		return -1;
	}
	if (!(nStop==1 || nStop==2))
	{
		HMI_ERROR("the nStop :[%d] is error!\n", nStop);
		return -1;
	}
	...
	{
		HMI_ERROR("com set error");
		return -1;
	}
	
	HMI_DEBUG("set successful!\n");
	return 0;
}
...
int serial_read(int fd, unsigned char *buf, size_t len)
{
	...
	if (buf==NULL)
	{
		HMI_ERROR("the parm buf:[%s] is error!\n", buf);
		return -1;
	}

	if (len < 0)
	{
		HMI_ERROR("the param len :[%d] is error!\n");
		return -1;
	}
	tout.tv_sec = 1;
