...
int gpio_open(int arg)
{
	...
	if (arg < 1 || arg > 11)
	{
		printf("the param :[%d] arg error!\n", arg);
		return -1;
	}

	memset(data, 0, 8);
	memset(gpio_value_path, 0, 128);
	select_num(arg, data);
...
int gpio_setio(int arg, int direction)
{
	...
	if (arg < 1 || arg > 11)
	{
		printf("the param arg :[%d] is error!\n", arg);
		return -1;
	}
	if (direction!=0 && direction!=1)
	{
		printf("the param direction :[%d] is error!\n", direction);
		return -1;
	}
	...
	printf("the direc_path:[%s]\n", direc_path);

	//set the gpio direction
	fd_direc = open(direc_path, O_WRONLY);
	if (fd_direc < 0)
	{
		printf("open direc_path:[%s] error!\n", direc_path);
		return -1;
	}
...
* 	int gpio_read(int arg, char *data)
...
*   @note    
*/
int gpio_read(int arg, char *data, int size)
{
	int ret;
	...
	if (data==NULL)
	{
		printf("the param NULL!\n");
		return -1;
	}
	if (size < 0)
	{
		printf("the param size:[%d] is error!\n");
		return -1;
	}

	//read the gpio
	ret = read(arg, data, size);
	if (ret < 0)
	{
		printf("read error!\n");
		return -1;
...
* 	int gpio_write(int arg, char *data)
...
int gpio_write(int arg, char *data)
{
	...
	//write the gpio
	ret = write(arg, data, sizeof(data));
	if (ret < 0)
	{
		printf("read error!\n");
		return -1;
